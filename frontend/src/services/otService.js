...
const otService = {
    ...
    cancel: async (publicId, payload) => (await apiClient.post(`/hospital/surgeries/${publicId}/cancel`, payload || {})).data,

    // --- OT recovery / PACU (Phase 8) ---
    close: async (publicId) => (await apiClient.post(`/hospital/surgeries/${publicId}/close`)).data,
    getRecovery: async (surgeryId) => (await apiClient.get(`/hospital/ot/surgeries/${surgeryId}/recovery`)).data,
    admitRecovery: async (surgeryId) => (await apiClient.post(`/hospital/ot/surgeries/${surgeryId}/recovery/admit`)).data,
    observeRecovery: async (surgeryId, payload) => (await apiClient.post(`/hospital/ot/surgeries/${surgeryId}/recovery/observe`, payload)).data,
    dischargeRecovery: async (surgeryId, destination) => (await apiClient.post(`/hospital/ot/surgeries/${surgeryId}/recovery/discharge`, { destination })).data,

    // --- OT execution: WHO checklist, milestones, operative note (Phase 7) ---
    getWhoChecklist: async (surgeryId) => (await apiClient.get(`/hospital/ot/surgeries/${surgeryId}/who-checklist`)).data,
    signWhoPhase: async (surgeryId, phase, payload) => (await apiClient.post(`/hospital/ot/surgeries/${surgeryId}/who-checklist/${phase}/sign`, payload || {})).data,
    getMilestones: async (surgeryId) => (await apiClient.get(`/hospital/ot/surgeries/${surgeryId}/milestones`)).data,
    recordMilestone: async (surgeryId, payload) => (await apiClient.post(`/hospital/ot/surgeries/${surgeryId}/milestones`, payload)).data,
    saveOperativeNote: async (surgeryId, note) => (await apiClient.post(`/hospital/ot/surgeries/${surgeryId}/operative-note`, { note })).data,
    ...
    getTeam: async (surgeryId) => (await apiClient.get(`/hospital/ot/surgeries/${surgeryId}/team`)).data,
    assignTeamMember: async (surgeryId, payload) => (await apiClient.post(`/hospital/ot/surgeries/${surgeryId}/team`, payload)).data,
    removeTeamMember: async (surgeryId, memberId) => (await apiClient.delete(`/hospital/ot/surgeries/${surgeryId}/team/${memberId}`)).data,
    ...
    applyArchetype: async (name) => (await apiClient.post(`/hospital/ot/policies/archetype/${name}`)).data,
    resetPolicies: async () => (await apiClient.post('/hospital/ot/policies/reset')).data,
    getOtAnalytics: async (date) => (await apiClient.get('/hospital/ot/analytics/summary', { params: date ? { date } : {} })).data,
    getOtNabh: async (from, to) => (await apiClient.get('/hospital/ot/analytics/nabh', { params: { ...(from ? { from } : {}), ...(to ? { to } : {}) } })).data,
    approve: async (publicId) => (await apiClient.post(`/hospital/surgeries/${publicId}/approve`)).data,
    ...
    updateRoom: async (publicId, payload) => (await apiClient.put(`/hospital/ot/rooms/${publicId}`, payload)).data,
    deactivateRoom: async (publicId) => (await apiClient.delete(`/hospital/ot/rooms/${publicId}`)).data,

    // Reads that back the board, list, waiting list and case timeline.
    getOtList: async (date) => (await apiClient.get('/hospital/ot/surgeries/list', { params: date ? { date } : {} })).data,
    getWaitingList: async () => (await apiClient.get('/hospital/ot/surgeries/waiting-list')).data,
    getTimeline: async (publicId) => (await apiClient.get(`/hospital/ot/surgeries/${publicId}/timeline`)).data,
    getCancellationReasons: async () => (await apiClient.get('/hospital/ot/surgeries/cancellation-reasons')).data,
    postpone: async (publicId, payload) => (await apiClient.post(`/hospital/ot/surgeries/${publicId}/postpone`, payload || {})).data,
    ...
    getOtPermissionCatalogue: async () => (await apiClient.get('/hospital/ot/permissions/catalogue')).data,
    ...
    updateOtPermissionMatrix: async (matrix) => (await apiClient.put('/hospital/ot/permissions', matrix)).data,
