        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 shadow-sm p-6 mt-6 space-y-8">
            ...
            <div>
                ...
                <p className="text-sm text-gray-500 mb-4">
                    Which pages are included when you print at consultation complete. Turning a page
                    off leaves it out of that combined print.
                </p>
                <div className="border border-gray-200 rounded-xl overflow-hidden">
                    {PRINT_TOGGLES.map((t, i) => (
                        <div key={t.key} className={`flex items-center justify-between px-4 py-3 ${i > 0 ? 'border-t border-gray-100' : ''}`}>
                            <span className={`text-sm font-medium ${s[t.key] ? 'text-gray-900' : 'text-gray-400'}`}>{t.label}</span>
                            <button
                                type="button"
                            ...
                            >
                                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${s[t.key] ? 'translate-x-6' : 'translate-x-1'}`} />
                            </button>
                        </div>
                    ))}
                </div>
            ...
            <div>
                ...
                <p className="text-sm text-gray-500 mb-4">
                    When this hospital collects the consultation + case-paper fee. Anything the doctor
                    adds during the consultation (procedures, in-clinic medicines, services) is added to
                    the same bill either way, and any balance is collected at checkout.
                </p>
                <div className="space-y-2">
                    {[
                        ...
                        <label key={opt.value} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${s.billPaymentTiming === opt.value ? 'border-gray-900 bg-gray-50' : 'border-gray-200 hover:bg-gray-50'}`}>
                            <input
                                type="radio"
