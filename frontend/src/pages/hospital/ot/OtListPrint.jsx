                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={print} disabled={loading}
                            className="px-4 py-2 rounded-lg text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-700">Print</button>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {rows.map((r) => (
                                    <tr key={r.publicId}>
                                        ...
                                        <td className="py-2">{r.patientName || '—'}{r.patientAge != null ? ` (${r.patientAge})` : ''}</td>
                                        <td className="py-2">{r.procedureName || '—'}</td>
                                        <td className="py-2">{r.surgeonName || '—'}</td>
                                    </tr>
                                ))}
                                {rows.length === 0 && <tr><td colSpan={5} className="text-center text-gray-400 py-8">No cases scheduled.</td></tr>}
                            </tbody>
                        </table>
                    )}
                </div>
