...
const fmt = (dt) => dt ? new Date(dt).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }) : null;
    ...
    const phaseRow = (phase, label, at, enabled, payload) => (
        <div className="flex items-center justify-between py-2">
            ...
            {at ? (
                ...
                <button disabled={busy || !enabled} onClick={() => signPhase(phase, payload)}
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center z-50 p-4 overflow-y-auto" onClick={onClose}>
            <div className="bg-white rounded-2xl w-full max-w-xl my-8" onClick={(e) => e.stopPropagation()}>
                ...
                {loading ? <div className="px-6 py-8 text-center text-gray-400">Loading…</div> : (
                    <div className="px-6 py-4 space-y-5">
                        <div className="border border-gray-200 rounded-lg p-3">
                            <h3 className="text-sm font-semibold text-gray-900 mb-1">WHO Surgical Safety Checklist</h3>
                            <div className="divide-y divide-gray-100">
                                {phaseRow('SIGN_IN', 'Sign-In (before induction)', who?.signInAt, true, { siteMarked: true })}
                                {phaseRow('TIME_OUT', 'Time-Out (before incision)', who?.timeOutAt, !!who?.signInAt)}
                                {phaseRow('SIGN_OUT', 'Sign-Out (counts correct)', who?.signOutAt, !!who?.timeOutAt, { countsCorrect: true })}
                            </div>
                        </div>
                        ...
                        <div className="border border-gray-200 rounded-lg p-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {MILESTONES.map(([code, label]) => {
                                    const d = done(code);
                                    return (
                                        <button key={code} disabled={busy || !!d} onClick={() => record(code)}
