...
const SurgeryTeamModal = ({ surgery, onClose }) => {
    ...
    const nameFor = (m) => {
        ...
        const doc = surgeons.find((s) => String(s.userId) === String(m.userId) || String(s.doctorId) === String(m.userId));
        return doc ? doc.name : `User #${m.userId}`;
    };
    const labelFor = (code) => (roles.find((r) => r.code === code)?.label) || code;

    const add = async () => {
        if (!form.caseRoleCode) { toastError('Choose a role'); return; }
        const useExternal = form.staffId === '__EXTERNAL__';
        if (!useExternal && !form.staffId) { toastError('Choose a staff member or external operator'); return; }
        if (useExternal && !form.externalName.trim()) { toastError("Enter the external operator's name"); return; }
        setBusy(true);
        try {
            const created = await otService.assignTeamMember(surgery.surgeryId, {
                caseRoleCode: form.caseRoleCode,
                ...(useExternal ? { externalName: form.externalName.trim() } : { userId: Number(form.staffId) }),
            });
            ...
            <div className="bg-white rounded-2xl w-full max-w-xl my-8" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <div>
                        ...
                        <p className="text-xs text-gray-400">{surgery.patientName} · {surgery.procedureName}</p>
                    </div>
                        ...
                        <>
                                    ...
                                    {team.map((m) => (
                                        <div key={m.id} className="flex items-center justify-between px-3 py-2 text-sm">
                                            <div>
                                                ...
                                                <span className="ml-2 text-xs text-gray-400">{labelFor(m.caseRoleCode)}</span>
                                                {m.externalName && <span className="ml-2 text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded">external</span>}
                                            </div>
                                            <button disabled={busy} onClick={() => remove(m)} className="text-xs text-red-500 hover:text-red-700">Remove</button>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="bg-slate-50 border border-gray-200 rounded-lg p-3 space-y-2">
                                <div className="flex items-center justify-between">
                                    <label className="text-xs font-semibold text-gray-600">Add team member</label>
                                    <button type="button" onClick={addCustomRole} className="text-xs text-gray-500 hover:text-gray-700 underline">+ custom role</button>
                                </div>
                                <select value={form.caseRoleCode} onChange={(e) => set('caseRoleCode', e.target.value)}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                    <option value="">Select role…</option>
                                    {roles.map((r) => <option key={r.code} value={r.code}>{r.label}{r.custom ? ' (custom)' : ''}</option>)}
                                </select>
                                <select value={form.staffId} onChange={(e) => set('staffId', e.target.value)}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                    <option value="">Select staff…</option>
                                    {surgeons.map((s) => <option key={s.doctorId} value={s.userId || s.doctorId}>{s.name}</option>)}
                                    <option value="__EXTERNAL__">External operator…</option>
                                </select>
                                {useExternal && (
                                    <input value={form.externalName} onChange={(e) => set('externalName', e.target.value)}
