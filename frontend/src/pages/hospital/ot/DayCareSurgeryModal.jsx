...
const DayCareSurgeryModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        if (!isOpen) return;
        setSearch(''); setPatients([]); setPatient(null);
        setProcedureName(''); setClinicalNotes('');
        setPriority('ELECTIVE'); setPreferredDate(''); setSubmitting(false);
    }, [isOpen]);

    useEffect(() => {
        if (!isOpen || search.trim().length < 2) { setPatients([]); return; }
        let active = true;
        const timer = setTimeout(() => {
            hospitalService.getPatients(search.trim(), 0, 8)
                .then((res) => { if (active) setPatients(res?.content || []); })
            ...
            <div className="bg-white rounded-2xl w-full max-w-xl my-8" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100">
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">A same-day procedure — the patient is not admitted.</p>
                    </div>
                        ...
                        {patient ? (
                            <div className="flex items-center justify-between border border-gray-300 rounded-lg px-3 py-2">
                                <span className="text-sm font-semibold text-gray-800">
                                    {patient.name} <span className="text-gray-400 font-normal">· {patient.customId || patient.id}</span>
                                </span>
                                <button type="button" onClick={() => setPatient(null)}
                                    className="text-xs text-gray-500 hover:text-gray-700 underline">Change</button>
                            </div>
                        ) : (
                            <>
                                <input
                                ...
                                {patients.length > 0 && (
                                    <div className="mt-1 border border-gray-200 rounded-lg divide-y divide-gray-100 max-h-48 overflow-y-auto">
                                        {patients.map((p) => (
                                            <button key={p.id} type="button" onClick={() => { setPatient(p); setPatients([]); }}
