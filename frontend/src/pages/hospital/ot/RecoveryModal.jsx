                    ...
                    {loading ? <div className="text-center text-gray-400 py-6">Loading…</div>
                        : !episode ? (
                            <div className="text-center space-y-3">
                                ...
                                <button disabled={busy} onClick={admit}
                            ...
                            <>
                                <div className="text-xs text-gray-500">
                                    ...
                                    {episode.dischargedAt && <> · Discharged {fmt(episode.dischargedAt)} → {episode.transferDestination}</>}
                                </div>
                                    ...
                                    <>
                                        <div className="flex items-end gap-2">
                                            <div className="flex-1">
                                                ...
                                                <input type="number" min="0" max="10" value={aldrete} onChange={(e) => setAldrete(e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                                            </div>
                                            <button disabled={busy} onClick={observe}
                                        ...
                                        <div className="flex items-end gap-2 border-t border-gray-100 pt-3">
                                            <div className="flex-1">
                                                <label className="block text-xs font-medium text-gray-600 mb-1">Discharge to</label>
                                                <select value={destination} onChange={(e) => setDestination(e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                                    {DESTINATIONS.map((d) => <option key={d} value={d}>{d}</option>)}
                                                </select>
                                            </div>
                                            <button disabled={busy} onClick={discharge}
                                                className="px-4 py-2 rounded-lg text-sm font-semibold bg-gray-900 text-white hover:bg-gray-800">
                                                Discharge
                                            </button>
                                        </div>
