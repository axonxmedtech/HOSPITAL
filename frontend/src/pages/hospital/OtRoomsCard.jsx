...
const OtRoomsCard = () => {
    ...
    const add = async (roomName, sourceWardId) => {
        if (!roomName || !roomName.trim()) { toastError('Theatre name is required'); return; }
        setBusy(true);
        try {
    ...
    if (loading) return <div className="bg-white rounded-2xl border border-gray-200 p-6 text-gray-400">Loading…</div>;
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
            <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                    The operation theatres reception can schedule into. Turnover is the cleaning
                    and set-up time enforced between two cases in the same theatre.
                </p>
            </div>

            <div className="flex flex-wrap items-end gap-2">
                <div className="flex-1 min-w-[180px]">
                    <label className="block text-xs font-medium text-gray-600 mb-1">Theatre name</label>
                    <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. OT-1 / Main Theatre"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
                <div className="w-32">
                    <label className="block text-xs font-medium text-gray-600 mb-1">Turnover (min)</label>
                    <input type="number" min="0" value={turnover} onChange={(e) => setTurnover(e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
                <button type="button" disabled={busy} onClick={() => add(name)}
            ...
            {rooms.length > 0 ? (
                    ...
                    {rooms.map((r) => (
                        <div key={r.publicId} className="flex items-center justify-between px-4 py-2.5 text-sm">
                            <div>
                                ...
                                <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${r.status === 'AVAILABLE' ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>{r.status}</span>
                            </div>
                ...
                <p className="text-sm text-gray-400">No theatres yet. Add one above, or convert a suggested ward below.</p>
            )}

            {suggestions.length > 0 && (
                <div className="bg-amber-50 border border-amber-100 rounded-lg p-3">
                    <p className="text-xs font-semibold text-amber-800 mb-2">
                        Wards that look like theatres — confirm each (some, like "FOOT WARD", only match by chance):
                    </p>
                    <div className="flex flex-wrap gap-2">
                        {suggestions.map((w) => (
                            <button key={w.wardId} type="button" disabled={busy} onClick={() => add(w.wardName, w.wardId)}
