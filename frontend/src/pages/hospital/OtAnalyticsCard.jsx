...
const OtAnalyticsCard = () => {
    ...
    useEffect(() => { load(from, to); }, [from, to]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
            <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Surgical-care indicators over the selected period.</p>
                </div>
            ...
            {loading ? <div className="text-center text-gray-400 py-8">Loading…</div> : data && (
                <>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <Metric label="Completed" value={data.completed} />
                        <Metric label="Cancelled" value={data.cancelled} />
                        <Metric label="WHO compliance" value={data.whoCompliancePercent} suffix="%"
                            hint="all 3 phases signed" />
                        <Metric label="Cancellation rate" value={data.cancellationRatePercent} suffix="%" />
                        <Metric label="First-case on-time" value={data.firstCaseOnTimePercent} suffix="%"
                            hint="within 15 min of schedule" />
                        <Metric label="Avg turnover" value={data.averageTurnoverMinutes} suffix=" min" />
                        <Metric label="Occupied theatre-hrs"
                            value={data.occupiedTheatreMinutes != null ? (data.occupiedTheatreMinutes / 60).toFixed(1) : null} />
                        <Metric label="Unplanned returns" value={data.unplannedReturns} hint="within 30 days" />
                    </div>

                    {Array.isArray(data.cancellationsByReason) && data.cancellationsByReason.length > 0 && (
                        <div className="border border-gray-200 rounded-lg p-3">
                            <div className="text-sm font-semibold text-gray-900 mb-2">Cancellations by reason</div>
