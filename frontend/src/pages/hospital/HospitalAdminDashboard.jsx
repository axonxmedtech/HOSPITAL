...
const LabTechniciansTable = () => {
    ...
    async function load() {
        setLoading(true);
        try {
            const r = await getLabTechnicians({ size: 100 });
    ...
    async function handleDeactivate(publicId) {
        ...
        try {
            ...
            success('Lab technician deactivated');
            load();
        ...
        <div>
            {labTechs.length === 0 ? (
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {labTechs.map(lt => (
                            <tr key={lt.publicId || lt.id} className="hover:bg-gray-50">
                                <td className="px-4 py-3 font-mono text-xs text-blue-600">{lt.customId || '—'}</td>
