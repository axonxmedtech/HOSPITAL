...
const BranchAuditLogsView = ({ refreshKey = 0 }) => {
    ...
    const getActionBadgeColor = (action) => {
        ...
        if (act.includes('CREATE') || act.includes('ADD') || act.includes('SAVE')) return 'bg-green-50 text-green-700 border-green-200';
        if (act.includes('DELETE') || act.includes('REMOVE') || act.includes('BLOCK') || act.includes('DISPOSE')) return 'bg-red-50 text-red-700 border-red-200';
        if (act.includes('UPDATE') || act.includes('EDIT') || act.includes('RESET')) return 'bg-blue-50 text-blue-700 border-blue-200';
                                ...
                                <tr key={log.id || index} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-400 font-bold mt-0.5">{formatTime(log.timestamp)}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase border ${getActionBadgeColor(log.action)}`}>
                                            {log.action}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-gray-700 font-medium max-w-xs truncate" title={log.details}>
                                        {log.details || '-'}
                                    </td>
            ...
            {totalPages > 1 && (
                <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between">
                    <span className="text-xs text-gray-500 font-semibold">
                        Showing {(page * pageSize) + 1} - {Math.min((page + 1) * pageSize, logs.length)} of {logs.length} trails
                    </span>
                        ...
                        {Array.from({ length: totalPages }).map((_, i) => (
                            <button
                                ...
                                className={`px-2.5 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                                    page === i ? 'bg-gray-900 text-white shadow-sm' : 'border border-gray-300 hover:bg-white text-gray-600'
                                }`}
                            >
                                {i + 1}
                            </button>
