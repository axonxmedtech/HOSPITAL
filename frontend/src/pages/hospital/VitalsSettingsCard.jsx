...
const VitalsSettingsCard = () => {
    ...
    const addCustom = async () => {
        if (!name.trim()) { toastError('Enter a vital name'); return; }
        setAdding(true);
        try {
            const created = await vitalsService.addCustom(name.trim(), unit.trim());
            setVitals((list) => [...list, {
                key: created.vitalKey, label: created.label, unit: created.unit,
                type: 'TEXT', isCustom: true, enabled: true, publicId: created.publicId,
            }]);
            setName(''); setUnit('');
            success('Vital added');
        } catch (e) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-200/80 shadow-sm p-6 mt-6">
            ...
            <p className="text-sm text-gray-500 mb-5">
                Choose which vitals are taken when creating an OPD entry. Vitals that are off are hidden from the
                OPD form and are not printed on the case paper. You can add your own vitals and delete them later.
            </p>
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden mb-5">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {vitals.map((v) => (
                            <tr key={v.key} className={`border-b border-gray-100 last:border-0 ${!v.enabled ? 'bg-gray-50/60' : ''}`}>
                                <td className={`px-4 py-3 font-medium ${v.enabled ? 'text-gray-900' : 'text-gray-400'}`}>
                                    {v.label}
                                    {v.isCustom && <span className="ml-2 text-[10px] font-bold uppercase tracking-wider text-indigo-500">custom</span>}
                                </td>
                                ...
                                <td className="px-4 py-3">
                                    ...
                                    >
                                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${v.enabled ? 'translate-x-6' : 'translate-x-1'}`} />
                                    </button>
                                </td>
                                <td className="px-4 py-3">
                                    {v.isCustom ? (
                                        <button onClick={() => remove(v)} className="text-xs font-semibold text-red-600 hover:text-red-700">Delete</button>
                                    ) : (
            ...
            <div className="flex flex-wrap items-end gap-3">
                <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">New vital name</label>
                    <input
                        value={name}
