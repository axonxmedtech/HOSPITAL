    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-slate-50 border border-gray-200 rounded-lg p-3">
                ...
                {archetypes.map((a) => (
                    <button key={a} type="button" disabled={busy} onClick={() => applyArchetype(a)}
            ...
            <div className="divide-y divide-gray-100">
                {catalogue.map((p) => (
                    <div key={p.key} className="flex items-center justify-between py-3 gap-4">
                        ...
                        <select value={values[p.key] || p.defaultValue} onChange={(e) => setValue(p.key, e.target.value)}
                            className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm min-w-[140px]">
                            {p.values.map((v) => <option key={v} value={v}>{v.replace(/_/g, ' ')}</option>)}
                        </select>
                    </div>
                ))}
            </div>
