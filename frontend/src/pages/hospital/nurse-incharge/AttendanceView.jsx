...
const STATUS_LABEL = {
    ...
    LEAVE: 'Leave', HOLIDAY: 'Holiday', LATE: 'Late',
};
...
const AttendanceView = () => {
    ...
    const load = useCallback(() => {
        ...
        Promise.all([
            ...
            setRows((Array.isArray(sheet) ? sheet : []).map((r) => ({
                ...r,
                _status: r.status || '',
    ...
    const setRowField = (nurseProfileId, key, value) =>
        setRows((rs) => rs.map((r) => (r.nurseProfileId === nurseProfileId ? { ...r, [key]: value } : r)));

    const saveRow = async (r) => {
        if (!r._status) { toastError('Pick a status first'); return; }
    ...
    const addNurse = (id) => {
        ...
        setRows((rs) => [...rs, { nurseProfileId: n.id, nurseName: n.name, _status: '', _checkIn: '', _checkOut: '', _remarks: '' }]);
    };
    ...
    return (
        <div>
            <div className="flex flex-wrap items-end gap-3 mb-4">
                ...
                {addable.length > 0 && (
                    <div>
                        <label className="block text-xs font-semibold text-gray-600 mb-1">Add nurse</label>
                        <select value="" onChange={(e) => addNurse(e.target.value)}
                            className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            <option value="">Add a ward nurse…</option>
                            {addable.map((n) => <option key={n.id} value={n.id}>{n.name}</option>)}
                        </select>
                    </div>
                )}
            </div>
                    ...
                    <tbody>
                        {rows.length === 0 && (
                            <tr><td colSpan={7} className="px-3 py-8 text-center text-gray-400">
                                {loading ? 'Loading…' : 'No nurses scheduled. Use "Add nurse" to mark leave/holiday.'}
                            </td></tr>
                        )}
                        {rows.map((r) => (
                            <tr key={r.nurseProfileId} className="border-b border-gray-100">
                                ...
                                <td className="px-3 py-2">
                                    <select value={r._status} onChange={(e) => setRowField(r.nurseProfileId, '_status', e.target.value)}
                                ...
                                <td className="px-3 py-2">
                                    <input type="time" value={r._checkIn} onChange={(e) => setRowField(r.nurseProfileId, '_checkIn', e.target.value)}
                                        className="px-2 py-1 border border-gray-300 rounded text-xs" />
                                </td>
                                <td className="px-3 py-2">
                                    <input type="time" value={r._checkOut} onChange={(e) => setRowField(r.nurseProfileId, '_checkOut', e.target.value)}
                                        className="px-2 py-1 border border-gray-300 rounded text-xs" />
                                </td>
                                <td className="px-3 py-2">
                                    <input value={r._remarks} onChange={(e) => setRowField(r.nurseProfileId, '_remarks', e.target.value)}
                                        placeholder="Remark" className="w-32 px-2 py-1 border border-gray-300 rounded text-xs" />
                                </td>
                                <td className="px-3 py-2 text-right">
                                    <button onClick={() => saveRow(r)} disabled={savingId === r.nurseProfileId}
                                        className="px-3 py-1 text-xs font-semibold rounded-lg bg-gray-900 text-white hover:bg-gray-800 disabled:bg-gray-400">
                                        {savingId === r.nurseProfileId ? 'Saving…' : (r.attendancePublicId ? 'Update' : 'Save')}
                                    </button>
                                </td>
                            </tr>
                        ))}
