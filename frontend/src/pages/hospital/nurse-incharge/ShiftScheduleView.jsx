...
const ShiftScheduleView = () => {
    ...
    useEffect(() => {
        ...
        nurseService.getMyWards()
            ...
            .catch((e) => { if (active) toastError(e?.response?.data?.error || 'Failed to load wards'); })
    ...
    useEffect(() => {
        ...
        timeSlotService.listShiftTemplates(true)
            ...
            .catch((e) => { if (active) toastError(e?.response?.data?.error || 'Failed to load shift templates'); });
        return () => { active = false; };
    }, [toastError]);
    ...
    const templateNameFor = (schedule) => {
        ...
        if (t) return t.name;
        if (schedule.startTime && schedule.endTime) return `${hhmm(schedule.startTime)}–${hhmm(schedule.endTime)}`;
        return null;
    };
        ...
        <div className="space-y-4">
                            ...
                            <tr className="text-left text-xs font-semibold text-gray-500 border-b border-gray-200">
                                ...
                                {days.map((d, i) => (
                                    <th key={i} className="px-3 py-3 text-center whitespace-nowrap">
                                        {DAY_LABELS[i]}<br />
                                        <span className="font-normal text-gray-400">{d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</span>
                                    </th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {nurses.map((n) => (
                                <tr key={n.id} className="border-b border-gray-100 hover:bg-gray-50">
                                    <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap sticky left-0 bg-white">{n.name}</td>
                                    {days.map((d, i) => {
                                        const dateStr = toYMD(d);
                                        const schedule = scheduleFor(n.id, dateStr);
                                        const isEditing = editingCell && editingCell.nurseProfileId === n.id && editingCell.dateStr === dateStr;
                                        return (
            ...
            {rangeFillOpen && (
                <RangeFillModal
                    ...
                    onDone={() => { setRangeFillOpen(false); loadGrid(); }}
                />
            )}
        </div>
    );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">Days of Week (none = all days)</label>
                    ...
                    <div>
                        ...
                        >
                            ...
                            {templates.map((t) => (
                                <option key={t.publicId} value={t.publicId}>{t.name} ({hhmm(t.startTime)}–{hhmm(t.endTime)})</option>
                            ))}
                        </select>
                    </div>
                </div>
