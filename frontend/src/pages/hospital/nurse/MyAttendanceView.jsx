...
const STATUS_STYLE = {
    ...
    HOLIDAY: 'bg-purple-100 text-purple-700', LATE: 'bg-orange-100 text-orange-700',
};
const LABEL = { PRESENT: 'Present', ABSENT: 'Absent', HALF_DAY: 'Half Day', LEAVE: 'Leave', HOLIDAY: 'Holiday', LATE: 'Late' };
...
const MyAttendanceView = ({ refreshKey }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        attendanceService.getMine(iso(from), iso(to))
    ...
    if (rows.length === 0) return <div className="text-center text-gray-400 py-16">No attendance recorded.</div>;
                    ...
                    {rows.map((r) => (
                        <tr key={r.publicId} className="border-b border-gray-100">
                            ...
                            <td className="px-4 py-3">
                                <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${STATUS_STYLE[r.status] || 'bg-gray-100 text-gray-600'}`}>
                                    {LABEL[r.status] || r.status}
                                </span>
                            </td>
                            <td className="px-4 py-3 text-gray-500">{r.shiftStartTime ? `${hhmm(r.shiftStartTime)}–${hhmm(r.shiftEndTime)}` : '—'}</td>
