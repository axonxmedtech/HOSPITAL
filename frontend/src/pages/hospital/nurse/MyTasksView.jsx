...
const MyTasksView = ({ refreshKey }) => {
    ...
    const handleStart = async (publicId) => {
        ...
        try {
            ...
            const msg = err.response?.data?.error || err.response?.data?.message || err.response?.data || 'Failed to start task';
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            const msg = err.response?.data?.error || err.response?.data?.message || err.response?.data || 'Failed to complete task';
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <p className="text-xs text-gray-500">Track and update the status of your patient care tasks.</p>
            </div>
            ...
            {tasks.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {tasks.map((task) => (
                        <div key={task.publicId || task.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
                            <div className="space-y-3">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <span className={`px-2 py-0.5 text-xs font-semibold rounded border ${getPriorityBadge(task.priority)}`}>
                                            {task.priority}
                                        </span>
                                ...
                                <div className="text-xxs text-gray-400 space-y-1">
                                    <div>Created: <span className="font-medium text-gray-600">{fmt(task.createdAt)}</span></div>
                                    {task.dueDate && (
                                        <div>Due: <span className="font-medium text-red-600">{fmt(task.dueDate)}</span></div>
                                    )}
                                    {task.completedAt && (
                                        <div>Completed: <span className="font-medium text-green-600">{fmt(task.completedAt)}</span></div>
                                    )}
            ...
            {completionModal.isOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setCompletionModal({ isOpen: false, taskPublicId: null })}>
                    <div className="bg-white rounded-lg border border-gray-200 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
                        <div className="p-6">
                            <h3 className="text-lg font-bold text-gray-900 mb-3">Complete Task</h3>
                            <div>
                                <label className="block text-xs font-medium text-gray-600 mb-1">Completion Remarks (optional)</label>
                                <textarea
