...
const MedicationPanel = ({ admissionId }) => {
    ...
    const orderBadge = (c) => {
        ...
        if (s === 'STOPPED') return <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-red-100 text-red-700">STOPPED BY DOCTOR</span>;
        if (s === 'COMPLETED') return <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-gray-200 text-gray-600">COMPLETED</span>;
        return <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-green-100 text-green-700">ACTIVE</span>;
    };
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                {activeMeds.length === 0 ? (
                    <p className="text-sm text-gray-500">No active prescriptions available to administer.</p>
                ) : (
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label className="block text-xs font-medium text-gray-600 mb-1">Select Medicine</label>
                                <select value={selectedPrescriptionId} onChange={(e) => setSelectedPrescriptionId(e.target.value)}
                                    ...
                                    {activeMeds.map((p) => (
                                        <option key={p.prescriptionId} value={p.prescriptionId}>
                                            {p.medicineName} ({p.dosage || 'No dose'} · {p.route || 'Oral'}){p.pending ? ' — pending today' : ''}
                                        </option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-gray-600 mb-1">Administered Time</label>
                                <input type="datetime-local" value={administeredTime} onChange={(e) => setAdministeredTime(e.target.value)}
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                                    ...
                                    return (
                                        <tr key={c.prescriptionId} className={stopped ? 'bg-red-50/40' : ''}>
                                            <td className="px-5 py-3">
                                                <div className={`font-medium ${stopped ? 'text-gray-500 line-through' : 'text-gray-900'}`}>{c.medicineName}</div>
                                                {c.instructions && <div className="text-[11px] text-gray-400 italic">{c.instructions}</div>}
                                            </td>
                                            <td className="px-5 py-3 text-gray-600">{c.dosage || '—'}</td>
                                            <td className="px-5 py-3 text-gray-600">{c.frequency || '—'}</td>
                                            <td className="px-5 py-3 text-gray-600">
                                                <span className="px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-800 rounded">{c.route || 'ORAL'}</span>
                                            </td>
                                            <td className="px-5 py-3 text-gray-600">{courseText(c)}</td>
                                            <td className="px-5 py-3">{orderBadge(c)}</td>
                                            <td className="px-5 py-3">
                                                {c.pending
                                                    ? <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-100 text-amber-700">PENDING TODAY</span>
                                                    : (c.courseActive && c.administeredToday)
                                                        ? <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-green-100 text-green-700">GIVEN TODAY</span>
                ...
                <p className="px-5 py-2 text-[11px] text-gray-400 border-t border-gray-100">
                    Stopped medicines are kept for history — the doctor asked to stop them; they are not deleted.
                </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl">
                <div className="px-5 py-3 border-b border-gray-100">
                    <h3 className="font-bold text-gray-800 text-sm">Medication Administration History (MAR)</h3>
                </div>
                {history.length === 0 ? (
                            ...
                            <li key={mar.publicId || mar.id} className="px-5 py-4">
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        <span className="text-sm font-semibold text-gray-900">{medName(mar.prescriptionId)}</span>
                                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${marStatusColor(mar.status)}`}>{mar.status}</span>
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        Administered: <span className="font-medium">{fmt(mar.administeredTime)}</span>
                                        {mar.nurseUserId && (
                                            <span className="ml-2 font-normal text-gray-400">
                                                (By Nurse {mar.nurseUserId === currentUserId ? 'Me' : `ID: ${mar.nurseUserId}`})
                                            </span>
                                        )}
                                    </div>
                                    {mar.remarks && (
                                        <p className="text-xs text-gray-600 bg-gray-50 p-2 rounded mt-2 border border-gray-100 italic">Remarks: {mar.remarks}</p>
                                    )}
                                </div>
                            </li>
                        ))}
