...
export const buildSugarChartHtml = (entries, f, hospital) => {
    ...
    const d = (dt) => dt ? new Date(dt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' }) : '';
    const t = (dt) => dt ? new Date(dt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : '';

    // Oldest first for a chronological chart.
    const ordered = [...(entries || [])].sort((a, b) => new Date(a.recordedAt) - new Date(b.recordedAt));
    ...
    const blankRows = Array.from({ length: blanks }).map(() => '<tr><td class="dt">&nbsp;</td><td></td><td></td><td></td><td></td></tr>').join('');
...
const SugarChartPanel = ({ admissionId, readOnly = false }) => {
    ...
    const saveEdit = async (publicId) => {
        try {
            await nurseService.updateSugarEntry(publicId, { bloodSugar: editSugar.trim(), treatment: editTreatment.trim() });
        ...
        <fieldset disabled={readOnly} style={{ display: 'contents' }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">Blood Sugar</label>
                        <input value={bloodSugar} onChange={(e) => setBloodSugar(e.target.value)} placeholder="e.g., 180 mg/dL (F) / 240 (PP)"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">Treatment</label>
                        <input value={treatment} onChange={(e) => setTreatment(e.target.value)} placeholder="e.g., Inj. Actrapid 6 units S/C"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
                    </div>
                </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl">
                <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <button onClick={handlePrint} className="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700">
                        Print Sugar Chart
                    </button>
                </div>
                {loading ? (
                    ...
                    <table className="min-w-full text-sm">
                        <thead className="bg-gray-50">
                            <tr className="text-left text-xs font-semibold text-gray-500 border-b border-gray-200">
                                <th className="px-5 py-3">DATE / TIME</th><th className="px-5 py-3">BLOOD SUGAR</th>
                                <th className="px-5 py-3">TREATMENT</th><th className="px-5 py-3 text-right">ACTIONS</th>
                            </tr>
                        </thead>
                                ...
                                if (editingId === e.publicId) {
                                    return (
                                        <tr key={e.publicId}>
                                            ...
                                            <td className="px-5 py-3"><input value={editSugar} onChange={(ev) => setEditSugar(ev.target.value)} className="w-full px-2 py-1 border border-gray-300 rounded text-sm" /></td>
                                            <td className="px-5 py-3"><input value={editTreatment} onChange={(ev) => setEditTreatment(ev.target.value)} className="w-full px-2 py-1 border border-gray-300 rounded text-sm" /></td>
                                            <td className="px-5 py-3 text-right whitespace-nowrap">
                                                <button onClick={() => saveEdit(e.publicId)} className="text-xs font-semibold text-gray-900 mr-2">Save</button>
                                                <button onClick={() => setEditingId(null)} className="text-xs font-semibold text-gray-500">Cancel</button>
                                            </td>
                                        </tr>
                                    );
                                }
                                return (
                                    <tr key={e.publicId}>
                                        ...
                                        <td className="px-5 py-3 text-right whitespace-nowrap">
                                            {mine && (
                                                <>
                                                    <button onClick={() => { setEditingId(e.publicId); setEditSugar(e.bloodSugar || ''); setEditTreatment(e.treatment || ''); }}
                                                        className="text-xs font-semibold text-gray-500 hover:text-gray-900 mr-2">Edit</button>
                                                    <button onClick={() => remove(e.publicId)} className="text-xs font-semibold text-red-500 hover:text-red-700">Delete</button>
                                                </>
                                            )}
                                        </td>
                                    </tr>
