...
const WardCard = ({ ward, onViewBeds, onEdit, onDelete, deleting, inchargeOptions, onSetIncharge }) => {
    return (
        <div className="bg-white rounded-xl shadow p-4 flex flex-col justify-between">
            ...
            {onSetIncharge && (
                <div className="mt-3">
                    <label className="block text-xs font-medium text-slate-500 mb-1">Nurse Incharge</label>
                    <select
                    ...
                    >
                        ...
                        {(inchargeOptions || []).map(n => (
                            <option key={n.nurseProfileId} value={n.nurseProfileId}>{n.name}</option>
                        ))}
                    </select>
                </div>
            )}
