...
| 38  | 🟠  | The nurse assignment rule was applied to uploads but not to reads                                                                                | Documents would have been the **only** clinical record a staff nurse could open for a patient they are not assigned to — vitals, notes and medication are all gated. Found because the implementer answered a direct question about it rather than assuming |
| 39  | 🟠  | `applyTheatreCharge` returned silently for a day-care case (no IPD admission) and for an admission with no bill open yet                         | The theatre was used and **never billed**, with nothing written anywhere. A pure revenue leak, invisible until an annual audit. Now logged at WARN with the amount, surgery and theatre; routing the fee to a day-care bill remains open work               |
| 40  | 🟡  | `hms.import.max-file-size` was parsed on every request instead of at startup — the twin of #34, knowingly left unfixed                           | A typo passed boot and then failed **every import**, mid-migration, when the admin is least able to debug it. Fixing one half of a defect class and leaving the other is how the second half gets found by a customer                                       |
| 41  | 🟡  | A `@Deprecated ADMITTABLE_TYPES` alias was left behind pointing at `INITIAL_ADMISSION_TYPES`, and the ICU billing test still asserted through it | The test read as though it pinned admission rules while actually pinning transfer rules. **The same "one thing, two names" shape as #3/#18/#23** — introduced by me while fixing that very category                                                         |
...
#41 is the same shape again — and it was introduced _while fixing_ that category, by leaving a
compatibility alias nobody needed. A deprecated alias inside one codebase with one caller is not
backward compatibility, it is the duplicate re-created under a politer name.

## Defects introduced by the fixes

#39, #40 and #41 were all self-inflicted during this work rather than found in pre-existing code,
and #40 was **knowingly deferred** at the time it was first spotted. That deferral is the lesson: a
defect class fixed in one place and left in another is not half-fixed, because the unfixed half no
longer has anything pointing at it. When the same shape appears twice, both get fixed in the same
change or the second one is lost.
